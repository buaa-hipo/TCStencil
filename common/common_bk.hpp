...
template <> half get_random<half>() {
    return __float2half(((double)rand()) / (RAND_MAX - 1));
}

template <typename T> static T *getRandom1DArray(int width_y) {
    T(*a) = (T(*)) new T[width_y];
...
template <typename T> T *getRandom2DArray(int width_y, int width_x) {
    T(*a)[width_y] = (T(*)[width_x]) new T[width_y * width_x];
    for (int j = 0; j < width_y; j++)
...
template <> inline half *getRandom2DArray(int width_y, int width_x) {
    half(*a)[width_y] = (half(*)[width_x]) new half[width_y * width_x];
    for (int j = 0; j < width_y; j++)
        for (int k = 0; k < width_x; k++) {
            a[j][k] = get_random<half>();
        }
    return (half *)a;
}
...
static T *getRandom3DArray(int height, int width_y, int width_x) {
    T(*a)[514][514] = (T(*)[514][514]) new T[height * 514 * 514];
...
static T *getRandom4DArray(int depth, int height, int width_y, int width_x) {
    ...
    [height][width_y][width_x] =
        (T(*)[height][width_y][width_x]) new T[depth * height * width_y *
                                               width_x];
...
template <typename T> static T *getZero1DArray(int width_x) {
    T(*a) = (T *)new T[width_x];
    memset((void *)a, 0, sizeof(T) * width_x);
    return (T *)a;
}

template <typename T> static T *getZero2DArray(int width_y, int width_x) {
    T(*a)[8196] = (T(*)[8196]) new T[8196 * 8196];
...
template <typename T>
static double checkError1D(int width_x, const T *output, const T *reference,
                           int x_lb, int x_ub) {
    double error = 0.0;
...
template <typename T>
static double checkError2D(int width_x, const T *l_output, const T *l_reference,
                           int y_lb, int y_ub, int x_lb, int x_ub) {
    const T(*output)[8196] = (const T(*)[8196])(l_output);
    const T(*reference)[8196] = (const T(*)[8196])(l_reference);
    double error = 0.0;
    double max_error = TOLERANCE;
    int max_k = 0, max_j = 0;
...
template <typename T>
static double checkError3D(int width_y, int width_x, const T *l_output,
                           const T *l_reference, int z_lb, int z_ub, int y_lb,
                           int y_ub, int x_lb, int x_ub) {
    const T(*output)[514][514] = (const T(*)[514][514])(l_output);
    const T(*reference)[514][514] = (const T(*)[514][514])(l_reference);
    double error = 0.0;
    ...
    for (int i = z_lb; i < z_ub; i++)
        for (int j = y_lb; j < y_ub; j++)
            for (int k = x_lb; k < x_ub; k++) {
                ...
                if (curr_error > max_error) {
                    printf(
                        "Values at index (%d,%d,%d) differ : %.6f and %.6f\n",
