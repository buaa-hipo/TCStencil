...
void init_coe(half *__restrict__ B, half *__restrict__ coe, int n) {
    ...
    for(int z = 0; z < 5; z++){
        ...
        for(int i = 0; i < 12; i++){
            for(int j = 0; j < 5; j++){
                coe[offset + 17 * i + 16 * j + 2] = B[z * 5 + j];
            }
        }
        // full part coe
        for (int i = 0; i < 3; i++) {
            coe[offset + i * 16] = B[z * 5 + i + 2];
            coe[offset + i * 16 + 223] = B[z * 5 + i];
        }
        for (int i = 0; i < 4; i++) {
            coe[offset + i * 16 + 1] = B[z * 5 + i + 1];
            coe[offset + i * 16 + 206] = B[z * 5 + i];
        }
    }
}
...
extern "C" void host_code(float *h_A, float *h_B, float *h_C, int N, int n) {
    ...
    cudaMalloc(&coe, sizeof(half) * 16 * 16 * 5);
    check_error("Failed to allocate device memory for coe");
    cudaMemcpy(coe, h_coe, sizeof(half) * 16 * 16 * 5, cudaMemcpyHostToDevice);
