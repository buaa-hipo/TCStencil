...
void init_coe(half *__restrict__ B, half *__restrict__ a, half *__restrict__ b,
    ...
    half vertical_buf[5], horizon_buf[5];
    for (int i = 0; i < 5; i++) {
        horizon_buf[i] = B[2 * n + i];
        vertical_buf[i] = B[2 + i * n];
    }
    horizon_buf[2] = __float2half(0);

    for (int i = 0; i < 12; i++) {
        for (int j = 0; j < 5; j++) {
            a[i * 16 + i + j + 32] = vertical_buf[j];
            b[i * 16 + i + j * 16 + 2] = horizon_buf[j];
        }
    }
    // full part coe
    for (int i = 0; i < 3; i++) {
        a[i] = vertical_buf[i + 2];
        a[i + 253] = vertical_buf[i];
        b[i * 16] = horizon_buf[i + 2];
        b[i * 16 + 15 + 208] = horizon_buf[i];
    }
    for (int i = 0; i < 4; i ++) {
        a[i + 16] = vertical_buf[i + 1];
        a[i + 236] = vertical_buf[i];
        b[i * 16 + 1] = horizon_buf[i + 1];
        b[i * 16 + 14 + 192] = horizon_buf[i];
    }
}
...
void set_halo_index(int *index1, int *index2, int N) {
    for (int i = 0; i < 32; i++) {
        ...
        index1[i + 64] = (x << 1) + y + 64;
        index2[i + 64] = (x << 4) - 242 + y;

        index1[i + 96] = (x << 1) + y + 96;
        index2[i + 96] = (x << 4) + 256 + y;
    }
}
...
extern "C" void host_code(float *h_A, float *h_B, float *h_C, int N, int n) {
    ...
    half *half_B = array_float2half(h_B, 5 * 5);
    init_coe(half_B, h_coe_a, h_coe_b, n);

    int h_index1[128], h_index2[128];
    ...
    int *index1, *index2;
    cudaMalloc(&index1, sizeof(int) * 128);
    check_error("Failed to allocate device memory for index1");
    cudaMalloc(&index2, sizeof(int) * 128);
    check_error("Failed to allocate device memory for index2");
    cudaMemcpy(index1, h_index1, sizeof(int) * 128, cudaMemcpyHostToDevice);
    cudaMemcpy(index2, h_index2, sizeof(int) * 128, cudaMemcpyHostToDevice);
